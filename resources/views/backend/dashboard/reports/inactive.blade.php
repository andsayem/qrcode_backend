<div class="card shadow-sm border-0">
    ...
    @php
        ...
        $start = \Carbon\Carbon::parse($fromDate)->startOfMonth();
        $end   = \Carbon\Carbon::parse($toDate)->endOfMonth();

        $current = $start->copy();

        while ($current <= $end) {
            $months[] = [
                'alias' => strtolower($current->format('M')) . '_' . $current->year . '_qty',
    ...
    <div class="card-header bg-light">
        <h4 class="mb-0 fw-bold" style="color:#1d3557;">
            Division Wise - <span style="color:#000;">(Inactive User)</span>
        </h4>
    </div>
